...
class CampoByIdRestControllerTest {
	...
	@Autowired
	CampoByIdRestController campoByIdRestController;
	
	@MockBean
	CampoRepo campoRepo;
	@MockBean
	LineaRepo lineaRepo; // para evitar errores en la carga del contexto de forma automatica
	@MockBean
	CampoRepresentationModelAssembler crma;
	
	
	@Test
	void testGetCampoById() {
		
		//WebTestClient client = webTestClient.mutateWith(configurer);
		WebTestClient client = WebTestClient.bindToController(campoByIdRestController).build().mutateWith(configurer);
