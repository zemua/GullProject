...
@SpringBootTest
class PvpSumForLineFinderFactoryTest {

	@Autowired PvpSumForLineFinderFactory factory;
	...
	void test() {
		...
		var mapper = factory.from(propuesta);
		
		assertEquals(140.18, mapper.getByDupla(l1, sum1.getId()));
		assertEquals(140.18, mapper.getByDupla(l1, sum1.getId()));
		assertEquals(85.6, mapper.getByDupla(l2, sum2.getId()));
		assertEquals(0.0, mapper.getByDupla(l1, sum2.getId()));
	}
